...
[LEARN:architecture] Three theme dirs, two brands: `ucdavis_beamer_theme_xelatex` (XeLaTeX, Verdana) and `ucdavis_beamer_theme_pdflatex` (pdfLaTeX, default serif) are the SAME theme named `ucdavis` — only the font setup differs. `ca_ed_lab_beamer_theme` is theme `caedlab` (pdfLaTeX, Carlito) — a port of the official CEL pptx; it HAS TOC styling + an `\AtBeginSection` section divider but NO progress bar. [updated: caedlab was formerly XeLaTeX/Helvetica and simpler]
...
[LEARN:gotcha] The hard-coded absolute-coordinate layout (bands at fixed TikZ y-coords assuming `aspectratio=169`, absolute frametitle/number coords, hard `\vskip`s + forced `\LARGE` title) is now FIXED for `ucdavis_beamer_theme_xelatex` (redesigned) and `ca_ed_lab_beamer_theme` (ported, fully paper-size-derived). It REMAINS only in `ucdavis_beamer_theme_pdflatex` (still uses 2.5 footer, 7.55/7.65 header etc.) — the last improvement target; port the xelatex redesign down to it (only font setup differs).
...
[LEARN:decision] CA Ed Lab redesign DONE (see the CA Education Lab theme entry near the top of this file) — it was reworked as a from-scratch port of the official CEL pptx rather than a UC Davis re-skin. The only remaining redesign target is `ucdavis_beamer_theme_pdflatex` (still the old hard-coded layout).
